...
import { Menu, X, ChevronLeft, ChevronRight, Home, Users, Settings } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 ${open ? "w-64" : "w-16"} 
          bg-green-500 text-white p-4 transform
          ${mobile ? "translate-x-0" : "-translate-x-full"}
          transition-all duration-300 md:relative md:translate-x-0`}
        
      >
        <div className="flex items-center justify-between">
          <h1 className={`font-bold mb-6 transition-all ${open ? "text-xl" : "hidden"}`}>
          Campo Digital
          </h1>
        ...
        <nav className="flex flex-col gap-4 mt-6">
          ...
          <a href="/ajustes" className="flex items-center gap-2 hover:text-gray-300">
